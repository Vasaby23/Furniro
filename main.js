...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (getLikeCookie('likeDivState') === 'show') {
        likeDiv.classList.add('show');
    ...
    btnLike.addEventListener('click', function(event) {
        ...
        if (likeDiv.classList.contains('show')) {
            likeDiv.classList.remove('show');
            likeDiv.classList.add('hidden');
            setLikeCookie('likeDivState', 'hidden');
        } else {
            likeDiv.classList.remove('hidden');
            likeDiv.classList.add('show');
            setLikeCookie('likeDivState', 'show');
        }
    });
    ...
    for (let i = 1; i <= 8; i++) {
        const likeLink = document.querySelector(`.a-like-${i}`);
        if (likeLink) {
            likeLink.addEventListener('click', function(event) {
                event.preventDefault();
                window.location.reload();
